			...
			foreach($lugares as $lugar) {
				if($i == 0) {
					echo "<li class='ui-state-default' id='{$lugar->id}'>";
						echo "<span class='ui-icon ui-icon-arrowthick-2-n-s'></span>({$lugar->id}) <strong>{$lugar->title}</strong> | Nr: <strong>{$lugar->logr_nr}</strong> (Inicia o mapa)";
...
function inlocation_cad_cep(){
	...
	if($_POST){
		...
		$url="https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($local). "&key=" . get_option('inlocation_googleapi')  . "&sensor=false";
...
function inlocation_endbycep(){
	...
	$jsonString = @file_get_contents($str="https://viacep.com.br/ws/{$cep}/json");

	wp_send_json(json_decode($jsonString));
}
